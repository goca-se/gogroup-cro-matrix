...
| Secret | Para quê |
|---|---|
| `ELEVATE_APICE`, `ELEVATE_BARBOURS`, `ELEVATE_KOKESHI`, `ELEVATE_RITUARIA`, `ELEVATE_LESCENT` | Um token por loja. Só as lojas com token são consultadas. |
...
2. Acrescente a linha em [`scripts/config.mjs`](scripts/config.mjs) (`STORES`).
3. Cadastre o secret `ELEVATE_<CHAVE>`.

Só isso. A loja vira coluna na matriz, entra no ranking de ganho acumulado e passa a contar na
média da vertical. As lojas do GoJump já estão comentadas no arquivo, prontas para descomentar.
